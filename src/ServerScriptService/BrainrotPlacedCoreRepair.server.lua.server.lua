--!nonstrict
-- ServerScriptService/BrainrotPlacedCoreRepair.server.lua
-- One core repair script for:
-- visible placed NPCs
-- money text slot matching
-- tool cleanup after placement
-- duplicate cleanup
...
local function makePlacedVisible(npc)
...
local function getVisibleBounds(model)
	...
	for _, obj in ipairs(model:GetDescendants()) do
		if obj:IsA("BasePart") and not isUtilityPart(obj) and obj.Size.Magnitude > 0.05 then
...
local function snapToStand(npc, standPart)
	makePlacedVisible(npc)

	local yaw = math.rad(standPart.Orientation.Y)
	npc:PivotTo(CFrame.new(npc:GetPivot().Position) * CFrame.Angles(0, yaw, 0))
	...
	local topY = standPart.Position.Y + standPart.Size.Y / 2

	local move = Vector3.new(
		standPart.Position.X - bounds.center.X,
		(topY + PLACE_Y_OFFSET) - bounds.min.Y,
...
local function getPlayerFromOwnerId(ownerId)
	for _, player in ipairs(Players:GetPlayers()) do
		if tostring(player.UserId) == tostring(ownerId) then
			return player
		end
	end
...
local function toolMatchesNpc(tool, npc)
	...
	if not npcId and getBrainrotName(tool) == getBrainrotName(npc) then
		return true
	end
...
local function removeToolsForPlacedNpc(npc)
	local ownerId = npc:GetAttribute("PlacedOwnerUserId") or npc:GetAttribute("OwnerUserId")
	local player = getPlayerFromOwnerId(ownerId)
	...
	for _, container in ipairs(containers) do
		if container then
			for _, tool in ipairs(container:GetChildren()) do
				if tool:IsA("Tool") and toolMatchesNpc(tool, npc) then
					tool:Destroy()
				end
			end
		end
...
local function removeDuplicatePlacedNpcs()
	...
	for _, npc in ipairs(npcFolder:GetChildren()) do
		if isPlacedNpc(npc) then
			...
			if ownerId ~= "" and strongId then
				key = ownerId .. "::ID::" .. strongId
			elseif ownerId ~= "" and slotId ~= "" then
				key = ownerId .. "::SLOT::" .. slotId
			end

			if key then
				groups[key] = groups[key] or {}
				table.insert(groups[key], npc)
			end
		end
	end
...
local function repair()
	...
	for _, npc in ipairs(npcFolder:GetChildren()) do
		...
		elseif isPlacedNpc(npc) then
			...
			if standPart then
				snapToStand(npc, standPart)
			else
				makePlacedVisible(npc)
			end

			removeToolsForPlacedNpc(npc)
		end
	end

	removeDuplicatePlacedNpcs()
end
...
print("[BrainrotPlacedCoreRepair] Loaded. Single repair script for placed NPCs/tools/visibility.")