...
local function finishEgg(player, egg, eggData)
	...
	local payload = {
		...
		selectedNPC = {
			id = tostring(tool:GetAttribute("BrainrotConfigId") or tool:GetAttribute("TemplateName") or tool:GetAttribute("DisplayName") or tool.Name),
			configId = tool:GetAttribute("BrainrotConfigId"),
			BrainrotConfigId = tool:GetAttribute("BrainrotConfigId"),
			modelName = tool:GetAttribute("ModelName") or tool:GetAttribute("BrainrotModelName"),
			ModelName = tool:GetAttribute("ModelName") or tool:GetAttribute("BrainrotModelName"),
...
local function damageEgg(player, egg, amount, maxDistance)
	...
	if egg:GetAttribute("CaptureChaseActive") ~= true then
		local chaseDuration = if eggDataForChase
			then tonumber(eggDataForChase.EggDef.ChaseTime) or 16
			else tonumber(egg:GetAttribute("CaptureChaseDuration")) or 16
