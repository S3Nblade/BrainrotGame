--!nonstrict
-- ServerScriptService/PlacedBrainrotVisualRepair.server.lua
-- Fixes placed brainrots that earn money but stay invisible.
-- Only affects placed NPCs. Inventory NPCs stay hidden.

local Workspace = game:GetService("Workspace")

local NPC_FOLDER_NAME = "BrainrotNPCs"
local REPAIR_EVERY = 0.2
local PLACE_Y_OFFSET = 0.32
...
local function forceNpcVisible(npc)
...
local function findStandForNpc(npc)
	...
	if nearestStand then
		...
		npc:SetAttribute("GeneratedFloorIndex", nearestStand.part:GetAttribute("GeneratedFloorIndex") or 1)
...
local function repairAllPlacedBrainrots()
...
task.spawn(function()
	...
	while true do
		repairAllPlacedBrainrots()
		task.wait(REPAIR_EVERY)
	end
end)

print("[PlacedBrainrotVisualRepair] Loaded. Invisible placed brainrots will be forced visible.")